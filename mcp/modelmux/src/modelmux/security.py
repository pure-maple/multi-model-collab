...
_PROMPT_INJECTION_PATTERNS: list[tuple[str, re.Pattern[str]]] = [
    ...
    ("role_override_ignore", re.compile(
        r"ignore\s+(all\s+)?(previous|prior|above|earlier)\s+(instructions?|prompts?|rules?|context)",
        re.IGNORECASE,
    )),
    ("role_override_disregard", re.compile(
        r"disregard\s+(all\s+)?(previous|prior|above|earlier)\s+(instructions?|prompts?|rules?)",
        re.IGNORECASE,
    )),
    ("role_override_forget", re.compile(
        r"forget\s+(all\s+)?(previous|prior|above|earlier)\s+(instructions?|prompts?|rules?)",
        re.IGNORECASE,
    )),
    ("role_override_you_are_now", re.compile(
        r"you\s+are\s+now\s+(?:a\s+)?(?:new|different|unrestricted|unfiltered)",
        re.IGNORECASE,
    )),
    ("role_override_new_instructions", re.compile(
        r"new\s+instructions?\s*:", re.IGNORECASE,
    )),
    ("role_override_system_colon", re.compile(
        r"^system\s*:", re.IGNORECASE | re.MULTILINE,
    )),
    ("role_override_system_header", re.compile(
        r"###\s*SYSTEM", re.IGNORECASE,
    )),
    ("role_override_override_all", re.compile(
        r"override\s+(all\s+)?(safety|security|content)\s+(filters?|policies|rules?|restrictions?)",
        re.IGNORECASE,
    )),
    ("role_override_do_not_follow", re.compile(
        r"do\s+not\s+follow\s+(your|the|any)\s+(previous|original|initial)\s+(instructions?|rules?|guidelines?)",
        re.IGNORECASE,
    )),
    ("role_override_from_now_on", re.compile(
        r"from\s+now\s+on,?\s+(you\s+)?(will|must|should|shall)\s+(ignore|disregard|forget|override)",
        re.IGNORECASE,
    )),

    # Delimiter injection
    ("delimiter_system_block", re.compile(
        r"```\s*system", re.IGNORECASE,
    )),
    ("delimiter_inst_tag", re.compile(
        r"\[/?INST\]", re.IGNORECASE,
    )),
    ("delimiter_llama_sys", re.compile(
        r"<<\s*SYS\s*>>", re.IGNORECASE,
    )),
    ("delimiter_chatml_system", re.compile(
        r"<\|im_start\|>\s*system", re.IGNORECASE,
    )),
    ("delimiter_chatml_end", re.compile(
        r"<\|im_end\|>", re.IGNORECASE,
    )),
    ("delimiter_human_turn", re.compile(
        r"<\|?\s*(?:human|user|assistant)\s*\|?>", re.IGNORECASE,
    )),
    ("delimiter_end_turn", re.compile(
        r"<\|endofturn\|>|<\|eot_id\|>", re.IGNORECASE,
    )),

    # Encoding evasion
    ("evasion_base64_instruct", re.compile(
        # base64 for common injection phrases — detect base64 blocks
        r"(?:base64|b64)\s*(?:decode|eval|exec)\s*[:(]",
        re.IGNORECASE,
    )),
    ("evasion_unicode_escape", re.compile(
        r"\\u[0-9a-fA-F]{4}.*\\u[0-9a-fA-F]{4}.*\\u[0-9a-fA-F]{4}",
    )),
    ("evasion_hex_escape", re.compile(
        r"\\x[0-9a-fA-F]{2}.*\\x[0-9a-fA-F]{2}.*\\x[0-9a-fA-F]{2}",
    )),
    ("evasion_rot13", re.compile(
        r"rot13|caesar\s+cipher\s+decode", re.IGNORECASE,
    )),

    # Social engineering
    ("social_pretend", re.compile(
        r"pretend\s+(you\s+are|to\s+be|you['re])\s+(?:a\s+)?(?:different|new|unrestricted|jailbroken)",
        re.IGNORECASE,
    )),
    ("social_act_as_if", re.compile(
        r"act\s+as\s+if\s+(you\s+)?(have\s+no|had\s+no|without)\s+(restrictions?|filters?|rules?|limitations?)",
        re.IGNORECASE,
    )),
    ("social_hypothetically", re.compile(
        r"hypothetically\s+if\s+you\s+had\s+no\s+(restrictions?|filters?|rules?|limitations?)",
        re.IGNORECASE,
    )),
    ("social_developer_mode", re.compile(
        r"(?:enable|activate|enter)\s+(?:developer|dev|debug|god|sudo|admin)\s+mode",
        re.IGNORECASE,
    )),
    ("social_jailbreak", re.compile(
        r"(?:DAN|STAN|DUDE|AIM)\s+(?:mode|prompt|jailbreak)",
        re.IGNORECASE,
    )),
    ("social_no_ethical", re.compile(
        r"(?:without|ignore|bypass|disable)\s+(?:your\s+)?(?:ethical|moral|safety)\s+(?:guidelines?|constraints?|programming)",
        re.IGNORECASE,
    )),

    # Output manipulation
    ("output_respond_only", re.compile(
        r"respond\s+only\s+with\s+(?:the\s+following|this|exactly)", re.IGNORECASE,
    )),
    ("output_exact", re.compile(
        r"output\s+the\s+following\s+exactly", re.IGNORECASE,
    )),
    ("output_repeat_after", re.compile(
        r"repeat\s+after\s+me\s*:", re.IGNORECASE,
    )),
    ("output_print_verbatim", re.compile(
...
_CREDENTIAL_LEAK_PATTERNS: list[tuple[str, re.Pattern[str]]] = [
    ...
    # Connection strings
    ("connection_string_mongo", re.compile(
        r"mongodb(?:\+srv)?://[^\s'\"]+:[^\s'\"]+@[^\s'\"]+",
    )),
    ("connection_string_postgres", re.compile(
        r"postgres(?:ql)?://[^\s'\"]+:[^\s'\"]+@[^\s'\"]+",
    )),
    ("connection_string_mysql", re.compile(
        r"mysql://[^\s'\"]+:[^\s'\"]+@[^\s'\"]+",
    )),
    ("connection_string_redis", re.compile(
        r"redis://:[^\s'\"]+@[^\s'\"]+",
    )),
...
_DATA_EXFIL_PATTERNS: list[tuple[str, re.Pattern[str]]] = [
    ("exfil_curl_post", re.compile(
        r"curl\s+.*-X\s*POST\s+.*https?://", re.IGNORECASE,
    )),
    ("exfil_wget_post", re.compile(
        r"wget\s+--post", re.IGNORECASE,
    )),
    ("exfil_nc_pipe", re.compile(
        r"\|\s*(?:nc|ncat|netcat)\s+", re.IGNORECASE,
    )),
]
