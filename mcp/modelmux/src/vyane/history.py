"""Dispatch result history for vyane.

Stores complete dispatch results (including full output) in:
  ~/.config/vyane/history.jsonl
...
def _history_file() -> Path:
    return resolve_user_write_path("history.jsonl")
...
def log_result(result_dict: dict, task: str = "", source: str = "dispatch") -> None:
    ...
    try:
        from vyane.notifications import notify_dispatch
...
def get_trends(hours: float = 24, bucket_minutes: int = 60) -> dict:
    ...
    for entry in raw:
        ...
        if usage:
            try:
                from vyane.costs import estimate_cost
