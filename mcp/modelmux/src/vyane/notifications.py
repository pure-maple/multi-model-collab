...
from vyane.config import _load_file, find_user_config_file

logger = logging.getLogger("vyane.notifications")
...
def load_notification_config() -> NotificationConfig:
    ...
    url = os.environ.get("VYANE_WEBHOOK_URL") or os.environ.get(
        "MODELMUX_WEBHOOK_URL",
        "",
    )
    if url:
    ...
    try:
        cfg_file = find_user_config_file()
        if cfg_file:
...
def _build_payload(result: dict, task: str, source: str, fmt: str) -> dict:
    ...
    title = f"{icon} Vyane {source}: {provider} ({status})"
