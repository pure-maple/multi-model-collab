...
_BENCHMARK_FILE = resolve_user_write_path("benchmark.json")
...
def _read_history_stats(hours: float = HISTORY_WINDOW_HOURS) -> dict[str, dict]:
    ...
    path = resolve_user_read_path("history.jsonl")
    if not path.exists():
        return {}
...
def smart_route(
    ...
    from vyane.feedback import feedback_scores as _feedback_scores
