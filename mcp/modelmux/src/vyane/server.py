"""Vyane — unified multi-model AI collaboration server.
...
Legacy CLI alias `modelmux` is kept for backward compatibility.
"""
...
from vyane.a2a import list_patterns as a2a_list_patterns
...
from vyane.audit import AuditEntry, count_recent, get_audit_stats, log_dispatch
...
from vyane.detect import CallerInfo, detect_caller, get_excluded_providers
from vyane.history import HistoryQuery, get_history_stats, log_result, read_history
...
from vyane.policy import check_policy, load_policy
from vyane.routing import classify_intent, smart_route
from vyane.security import ThreatLevel, scan_task
...
mcp = FastMCP(
    "Vyane",
    instructions=(
        "Vyane — multi-model AI collaboration server. Use mux_dispatch to send "
...
async def mux_dispatch(
    ...
    if not security_result.passed:
        from vyane.audit import log_security_event
...
async def mux_task_list() -> str:
    ...
    Reads status files from Vyane's status directory
    (with legacy modelmux fallback)
...
async def mux_history(
    ...
    if costs:
        from vyane.costs import aggregate_costs
...
async def mux_feedback(
    ...
    from vyane.routing import classify_task
...
async def mux_workflow(
    ...
    for cfg_file in [
        find_user_config_file(),
        _find_config_file(Path(resolved_workdir) / ".modelmux"),
    ]:
        if cfg_file:
            try:
                raw = _load_file(cfg_file)
...
async def mux_check(ctx: Context, diagnose: str = "") -> str:
    ...
    from vyane.feedback import _feedback_file
    from vyane.routing import _BENCHMARK_FILE
    ...
    if diagnose.strip():
        from vyane.routing import classify_task, smart_route
...
async def mux_collaborate(
    ...
    if provider_map:
        ...
        from vyane.a2a.patterns import get_pattern as _get_pattern
